...
def step(self: Trainer, variable: ϴ) -> Tuple[ϴ, Dict[str, X]]:
    ...
    grads, loss = jax.tree.map(F.partial(np.mean, axis=0),
    ...
    if self.cfg["clip"] is not None:
        ...
        grads = jax.tree.map(F.partial(jax.lax.cond, loss["norm"] < self.cfg["clip"],
    ...
    return {**variable, **dict(params=optax.apply_updates(params, updates))}, loss
...
def eval(self: Trainer, variable: ϴ) -> Tuple[Dict, X]:
    ...
    with jax.default_device(cpu:=jax.devices("cpu")[0]):
        ...
        return jax.tree.map(np.mean, jax.vmap(metric(self.pde, s))(ϕ, u, v)), (u, v)

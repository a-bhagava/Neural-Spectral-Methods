...
class CurledWake(PDE):
    def __str__(self):
        return f"CurledWake: {self.T}T, {self.nu}ν, {self.σ}σ"
    ...
    def equation(self, x: X, _u0: X, _u: X): 
        ...
        v, w = self.v, self.w
        nu_T = self.nu # fix to -- 10^-3 -- can this be learnt instead ? how does projection layer work? 
        _Dudx = _ux / self.X + v * _uy + w * _uz # self.X is the scaling factor 

        # forcing term -- is this the TKE? 
        if self.fn is None: f = np.zeros_like(_Dudx)
        else: f = self.fn(*_u[0].squeeze(-1).shape) # unsure if this is correct

        return (_Dudx - nu_T * _Δu - f) / self.u 
    ...
    def spectral(self, _u0: Basis, _u: Basis) -> Basis:
        ...
        v, w = jax.vmap(velocity_field)() 
        nu_T = self.nu  # unsure if i need to compute this just once or every time
        ...
        return self.basis.add(_Dudx, self.basis(-nu_T * _Δu.coef), f.map(np.negative)) / self.u
"""
Mixed-scale evaluation for NSM and UNet models.

Loads mixed-scale-trained models (e.g. "combined_8_4_2"), each with one or
more seeds, and evaluates every seed against every test length scale
(L=0.1 ... L=1.0). Saves per-seed predictions + metrics to disk, and an
aggregated mean/std across seeds for every (model_type, train_scale,
test_scale). Plotting is intentionally left out for now -- run this to get
metrics on disk first, plot separately once you know what you want to see.

Usage:
    python -m analysis.evaluate_mixed_scale --output-dir eval_results_mixed_scale_training
"""
...
# Make `src` importable no matter where this script is invoked from.
REPO_ROOT = Path(__file__).resolve().parent.parent
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from src.pde.navierstokes import (
    re4_1, re4_2, re4_3, re4_4, re4_6, re4_8, re4_10,
    re4_combined_8_4_2, re4_combined_8_4, re4_combined_8_2,
)
from src import *
from src.pde import *
...
# model_type -> {train_scale_name -> root_dir_containing_run(s)}
# NSM_ROOT_DIRS = {
#     "combined_8_4_2": Path("/home/abhagava/orcd/scratch/Neural-Spectral-Methods/log/mixed_scale_training/nsm/combined_8_4_2/"),
#     "combined_8_4":   Path("/home/abhagava/orcd/scratch/Neural-Spectral-Methods/log/mixed_scale_training/nsm/combined_8_4/"),
#     "combined_8_2":   Path("/home/abhagava/orcd/scratch/Neural-Spectral-Methods/log/mixed_scale_training/nsm/combined_8_2/"),
# }

    # "combined_8_4_2": Path("/home/abhagava/orcd/scratch/Neural-Spectral-Methods/log/mixed_scale_training/unet/combined_8_4_2/"),
    # "combined_8_4":   Path("/home/abhagava/orcd/scratch/Neural-Spectral-Methods/log/mixed_scale_training/unet/combined_8_4/"),

UNET_ROOT_DIRS = {
    "combined_8_2":   Path("/home/abhagava/orcd/scratch/Neural-Spectral-Methods/log/mixed_scale_training/unet/combined_8_2/"),
}
...
# Every test scale you want metrics against.
TEST_PDES = {
    "L=0.1": re4_1,
    "L=0.2": re4_2,
    "L=0.3": re4_3,
...
# Training PDE for each mixed-scale root-dir key.
TRAIN_PDES = {
    "combined_8_4_2": re4_combined_8_4_2,
    "combined_8_4":   re4_combined_8_4,
    "combined_8_2":   re4_combined_8_2,
}

MODEL_TYPES  = list(MODEL_ROOT_DIRS.keys())
TEST_SCALES  = list(TEST_PDES.keys())     # x-axis: all 7 test scales
TRAIN_GROUPS = list(MODEL_ROOT_DIRS["unet"].keys()) # facet panels: combined_8_4_2 / combined_8_4 / combined_8_2
...
def find_seed_runs(root_dir: Path) -> list[Path]:
    """
    Every subdirectory of root_dir that looks like a completed run (one per
    seed). Falls back to treating root_dir itself as the sole run if the
    checkpoint sits directly inside it (no per-seed subfolder layer).
    """
    if not root_dir.exists():
        return []
    if (root_dir / "variable_ckpt.npy").exists() and (root_dir / "cfg").exists():
        return [root_dir]
    runs = []
...
def is_in_dist(test_scale: str, train_scale: str) -> bool:
    """
    True if test_scale was seen during training. train_scale looks like
    "combined_8_4_2", meaning the model saw L=0.8, L=0.4, L=0.2 -- so a test
    scale is in-dist if its digit tag is one of the underscore-separated
    components of the combined name.
    """
    digit_tag = test_scale.replace("L=", "").replace(".", "").lstrip("0") or "0"
    return digit_tag in train_scale.split("_")
...
# ── plotting: single prediction snapshot (optional, off by default) ─────────
...
def evaluate_all(output_dir: Path, save_snapshots: bool = False) -> list[dict]:
    """
    For every (model_type, train_scale) pair, evaluates every seed run against
    every test scale. Saves per-seed predictions/metrics to disk. Returns a
    flat list of per-seed result rows.
    """
    ...
    for model_type, root_dirs in MODEL_ROOT_DIRS.items():
        for train_scale, root_dir in root_dirs.items():
            ...
            train_pde = TRAIN_PDES[train_scale]
            ...
            for seed_idx, run_dir in enumerate(seed_runs):
                ...
                for test_scale, test_pde in TEST_PDES.items():
                    in_dist = is_in_dist(test_scale, train_scale)
                    ...
                    try:
                        ...
                        if save_snapshots and seed_idx == 0:
                            plot_snapshot(
                                u_true, u_pred,
                        ...
                        rows.append(row)
                        print(f"errr={metrics.get('errr', float('nan')):.4f}"
    ...
    if rows:
        raw_path = output_dir / "mixed_scale_raw.csv"
...
def aggregate_across_seeds(rows: list[dict], output_dir: Path) -> dict:
    ...
    if summary_rows:
        summary_path = output_dir / "mixed_scale_summary.csv"
    ...
    with (output_dir / "mixed_scale_summary.json").open("w") as f:
...
def plot_cross_scale_bars(
    ...
    """
    One subplot per training scale (combined_8_4_2 / combined_8_4 /
    combined_8_2), grouped bars (NSM vs UNet) for each test scale, error bars
    from the std across seeds, and a star marking each in-distribution bar.
    """
    test_scales  = test_scales  or TEST_SCALES
    train_scales = train_scales or TRAIN_GROUPS
    ...
    out_path = output_dir / f"mixed_scale_{metric}_bars.png"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Mixed-scale NSM vs UNet evaluation (metrics only)")
    parser.add_argument("--output-dir", default="eval_results_mixed_scale_training")
    parser.add_argument(
        "--save-snapshots", action="store_true",
        help="Also save a ground-truth/prediction/error snapshot PNG for the first seed "
             "of each (train_scale, test_scale) pair. Off by default to keep this fast.",
    )
    parser.add_argument(
        "--bar-metrics", nargs="+", default=["residual"],
        help="Metric keys to render as grouped bar charts (one figure per metric). "
             "Pass an empty list (--bar-metrics) to skip plotting entirely.",
    )
    args = parser.parse_args()

    output_dir = Path(args.output_dir)
    rows = evaluate_all(output_dir, save_snapshots=args.save_snapshots)
    agg  = aggregate_across_seeds(rows, output_dir)

    print("\nDone. Metrics are in:")
    print(f"  {output_dir / 'mixed_scale_raw.csv'}      (per-seed, per-test-scale)")
    print(f"  {output_dir / 'mixed_scale_summary.csv'}  (mean/std across seeds)")
    print(f"  {output_dir / 'mixed_scale_summary.json'} (same, nested — feed this to plotting later)")

    for metric in args.bar_metrics:
        plot_cross_scale_bars(agg, metric, output_dir)